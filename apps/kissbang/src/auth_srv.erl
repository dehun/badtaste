...
init_db() ->
    ...
    mnesia:start(),
    mnesia:create_table(authinfo, [{disc_copies, [node() | nodes()]}, 
                              {attributes, record_info(fields, authinfo)}]),
    ok.

inner_register(Login, Password) ->
    Trans = fun() ->
                    Existance = qlc:q([X || X <- qlc:table(authinfo),
                                      X#authinfo.login == Login]),
                    case Existance of
                        [] ->
                            AuthInfo = #authinfo{guid = guid_srv:create(),
                                                 login = Login,
                                                 password = Password},
