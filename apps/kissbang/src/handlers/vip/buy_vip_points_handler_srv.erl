...
%%%-------------------------------------------------------------------
-module(buy_vip_points_handler_srv).
-include("../../kissbang_messaging.hrl").
-behaviour(gen_server).

%% API
-export([start_link/0]).
-export([handle_buy_vip_points/2]).
...
%%--------------------------------------------------------------------
init([]) ->
    handler_utils:register_handler(buy_vip_points, fun handle_buy_vip_points/2),
    {ok, #state{}}.
...
handle_buy_vip_points(CallerGuid, Message) ->
    ...
    case dtranse:dtranse([fun(Trans) -> vip_srv:buy_vip_points(CallerGuid, 1000, Trans) end,
        ...
        fail ->
            proxy_srv:async_route_messages(CallerGuid, [#on_vip_points_buy_fail{}])
    end.
