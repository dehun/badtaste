package ru.evast.integration.inner.VK 
{
	...
	public class VKInnerAdapter implements IDataAdapter 
	{
		...
		public function ShowPayment(count:int, mainName:String, code:int, additional:Object = null):void {
			...
			p.type = "item";
			p.item = code;
			_apiLib.callMethod("showOrderBox", p);
			//ExternalInterface.call(" VK.External.showOrderBox(type:'item', item:'" + code + "') ");
			//ExternalInterface.call(" VK.External.showPaymentBo(angry)" + count + ") ");
		}
				...
				_apiLib.api("friends.getAppUsers", { }, 
							function( o:Object ):void { 
													if(o.length == 0) {
																   ...
																   GetProfiles( o.join(","), callback );
													}
											 }, //!!! Не загрузит более 1000 друзей
											 onApiError );
		}
